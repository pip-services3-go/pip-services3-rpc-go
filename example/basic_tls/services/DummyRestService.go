...
type DummyRestService struct {
	*services.RestService
	controller     elogic.IDummyController
	numberOfCalls  int
...
func (c *DummyRestService) SetReferences(references crefer.IReferences) {
	...
	if depErr == nil && depRes != nil {
		c.controller = depRes.(elogic.IDummyController)
	}
}
...
func (c *DummyRestService) Register() {
	...
	c.RegisterRoute(
		"post", "/dummies",
		&cvalid.NewObjectSchema().
			WithRequiredProperty("body", edata.NewDummySchema()).Schema,
		c.create,
	)
